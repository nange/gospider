...
// nolint: dupl
func (qs TaskQuerySet) OutputSysDBIDIn(outputSysDBID ...uint64) TaskQuerySet {
	if len(outputSysDBID) == 0 {
		qs.db.AddError(errors.New("must at least pass one outputSysDBID in OutputSysDBIDIn"))
		return qs.w(qs.db)
	}
	return qs.w(qs.db.Where("output_sysdb_id IN (?)", outputSysDBID))
}
...
// nolint: dupl
func (qs TaskQuerySet) OutputSysDBIDNotIn(outputSysDBID ...uint64) TaskQuerySet {
	if len(outputSysDBID) == 0 {
		qs.db.AddError(errors.New("must at least pass one outputSysDBID in OutputSysDBIDNotIn"))
		return qs.w(qs.db)
	}
	return qs.w(qs.db.Where("output_sysdb_id NOT IN (?)", outputSysDBID))
}
